...
// // User data from 192.168.101.182, Patient data from lblerp.anantdv.com
...
//   // STEP 2: When we get the Card Blo Me Number, fetch Patient from PRODUCTION server (lblerp.anantdv.com)
//   useEffect(() => {
...
//       console.log("Patient API response:", parsedResult);
      
//       // Check if the response has message property (Frappe API structure)
//       if (parsedResult.message && parsedResult.message.length > 0) {
...
//       } 
//       // Fallback to check data property (in case response structure varies)
//       else if (parsedResult.data && parsedResult.data.length > 0) {
//         const patient = parsedResult.data[0];
...
//         setPatientError("Cannot connect to production server. Please check your connection.");
//       }
//     } finally {
...
//   // Function to fetch Lab Tests from PRODUCTION server (lblerp.anantdv.com)
...
//       const url = `https://lblerp.anantdv.com/api/resource/Lab Test?fields=["name","lab_test_name","template","status","result_date","practitioner_name","patient","patient_name","docstatus","invoiced"]&filters=[["patient","=","${patientId}"],["docstatus","=",1]]&order_by=result_date desc&limit_page_length=20`;
...
//         const result = await response.text();
//         const parsedResult = JSON.parse(result);
//         setLabTestData(parsedResult.data || []);
//         console.log("Lab tests fetched:", parsedResult.data?.length || 0);
//       }
//     } catch (error) {
...
//   // Function to fetch Appointments from PRODUCTION server (lblerp.anantdv.com)
...
//       const url = `https://lblerp.anantdv.com/api/resource/Patient Appointment?filters=[["patient","=","${patientId}"],["docstatus","=",1]]&fields=["*"]&order_by=appointment_date desc&limit_page_length=20`;
...
//         const result = await response.text();
//         const parsedResult = JSON.parse(result);
//         setAppointmentData(parsedResult.data || []);
//         console.log("Appointments fetched:", parsedResult.data?.length || 0);
//       }
//     } 
//     catch (error) {
...
//   // STEP 2: When we get the Card Blo Me Number, fetch Patient from PRODUCTION server
//   useEffect(() => {
//     if (userData && userData.card_blo_me_number) {
//       const userCardNumber = userData.card_blo_me_number;
//       setCardBloMeNumber(userCardNumber);
//       console.log("Fetching patient with card:", userCardNumber);
      
//       // Fetch patient from production server
//       fetchPatientFromProduction(userCardNumber);
//     }
//   }, [userData]);

//   // Function to fetch Patient from PRODUCTION server (lblerp.anantdv.com)
//   const fetchPatientFromProduction = async (cardNumber) => {
//     setPatientLoading(true);
//     setPatientError(null);
...
//       console.log("API Response:", parsedResult.message ? `Found ${parsedResult.message.length} patient(s)` : 'No patients found');
      
//       // Check if patient data exists
//       if (parsedResult.message && Array.isArray(parsedResult.message) && parsedResult.message.length > 0) {
//         const patient = parsedResult.message[0];
        
//         console.log("Patient loaded:", patient.name, "-", patient.patient_name);
...
//         setPatientError(null);
//       } else {
//         console.log("No patient found with card:", cardNumber);
//         setPatientError(`No patient found with Card Number: ${cardNumber}`);
//         setPatientData(null);
//         setPatientId(null);
//       }
//     } catch (error) {
//       console.error("Error fetching patient:", error.message);
//       setPatientError(`Error: ${error.message}`);
//       setPatientData(null);
//       setPatientId(null);
//     } finally {
//       setPatientLoading(false);
...
//   useEffect(() => {
//     if (patientId) {
//       console.log("Fetching lab tests and appointments for:", patientId);
//       fetchLabTestsFromProduction(patientId);
//       fetchAppointmentsFromProduction(patientId);
//     }
//   }, [patientId]);
...
//         const parsedResult = JSON.parse(result);
//         setLabTestData(parsedResult.data || []);
//         console.log("Lab tests loaded:", parsedResult.data?.length || 0);
//       }
//     } catch (error) {
...
//         const parsedResult = JSON.parse(result);
//         setAppointmentData(parsedResult.data || []);
//         console.log("Appointments loaded:", parsedResult.data?.length || 0);
//       }
//     } catch (error) {
...
// HealthBloMe.jsx - Improved version with better patient data fetching
// User data from local, Patient data from production
