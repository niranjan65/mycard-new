...
// HealthBloMe.jsx - Main Health Dashboard Component
// This is the main component that imports all sub-components
...
const HealthBloMe = () => {
  ...
  // First, fetch the User document to get Card Blo Me Number
  const { data: userData, isLoading: userLoading, error: userError } = useFrappeGetDoc("User", userEmail);

  // Extract Card Blo Me Number from user data
  useEffect(() => {
    if (userData && userData.card_blo_me_number) {
      setCardBloMeNumber(userData.card_blo_me_number);
      console.log("Card Blo Me Number found:", userData.card_blo_me_number);
    }
  }, [userData]);

  // Fetch patient data using Card Blo Me Number
  const { data: patientData, isLoading: patientLoading, error: patientError } = useFrappeGetDocList("Patient", {
    fields: ["*"],
    filters: cardBloMeNumber ? [["card_blo_me_number", "=", cardBloMeNumber]] : [["email", "=", userEmail]],
    limit_page_length: 1,
  });

  // Set patient ID when patient data is fetched
  useEffect(() => {
    if (patientData && patientData.length > 0) {
      setPatientId(patientData[0].name);
      console.log("Patient ID found:", patientData[0].name);
    }
  }, [patientData]);

  // Fetch Lab Test data for the current patient using patient ID
  const { data: labTestData, isLoading: labTestLoading, error: labTestError } = useFrappeGetDocList("Lab Test", {
    fields: [
      "name", "lab_test_name", "template", "status", "result_date", "time",
      "practitioner_name", "requesting_department", "patient", "patient_name",
      "docstatus", "expected_result_date", "department", "service_request",
      "submitted_date", "lab_test_group", "medical_code", "invoiced"
    ],
    filters: patientId ? [
      ["patient", "=", patientId],
      ["docstatus", "=", 1]
    ] : [],
    orderBy: { field: "result_date", order: "desc" },
    limit_page_length: 20,
  });

  // Fetch Patient Appointments for the current patient using patient ID
  const { data: appointmentData, isLoading: appointmentLoading, mutate: refreshAppointments } = useFrappeGetDocList("Patient Appointment", {
    fields: [
      "name", "appointment_date", "appointment_datetime", "appointment_time",
      "appointment_type", "practitioner", "practitioner_name", "department",
      "service_unit", "status", "patient", "patient_name", "title",
      "duration", "mode_of_payment", "paid_amount", "invoiced",
      "ref_sales_invoice", "billing_item", "procedure_template", "therapy_plan"
    ],
    filters: patientId ? [["patient", "=", patientId]] : [],
    orderBy: { field: "appointment_date", order: "desc" },
    limit_page_length: 20,
  });
  ...
  const labReports = labTestData ? labTestData.map((test, index) => ({
    ...
    labTestId: test.name,
    serviceRequest: test.service_request,
    submittedDate: test.submitted_date,
    labTestGroup: test.lab_test_group,
  ...
  const recentLabTests = labReports.slice(0, 3).map(report => ({
    ...
    status: report.status === 'Completed' ? 'completed' : 'pending',
    result: report.status === 'Completed' ? 'Normal' : '-',
    labTestId: report.labTestId // Add this for download functionality
  }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Manage your health records, appointments, and medical history</p>
        {cardBloMeNumber && (
          <p className="text-sm text-blue-600 mt-1">Card Blo Me Number: {cardBloMeNumber}</p>
        )}
      </div>
