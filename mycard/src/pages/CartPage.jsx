...
const CartPage = () => {
  ...
  // **🎯 FIXED Remove Item from Cart - Force immediate state update**
  const removeFromCart = async (itemCode, itemName) => {
    if (!window.confirm(`Are you sure you want to remove "${itemName}" from cart?`)) {
      return;
    }
    ...
    try {
      ...
      setCartItems(prevItems => {
        const newItems = prevItems.filter(item => item.item_code !== itemCode);
        console.log('🗑️ Immediately removed item from UI, remaining items:', newItems.length);
        return newItems;
      });

      // **🔧 Update cart summary immediately**
      setCartSummary(prev => {
        const removedItem = cartItems.find(item => item.item_code === itemCode);
        if (removedItem) {
          return {
            ...prev,
            total_items: Math.max(0, prev.total_items - removedItem.qty),
            total_amount: Math.max(0, prev.total_amount - removedItem.amount),
            net_total: Math.max(0, prev.net_total - removedItem.amount),
            grand_total: Math.max(0, prev.grand_total - removedItem.amount)
          };
        }
        return prev;
      });
      ...
      const success = await updateCartQuantity(itemCode, 0);
  ...
  // **🎯 FIXED Clear Entire Cart - Force immediate state update**
  const clearCart = async () => {
    if (!window.confirm('Are you sure you want to clear your entire cart?')) {
      return;
    }
    ...
    try {
      ...
      // **🔧 IMMEDIATELY clear UI state first**
      setCartItems([]);
      setCartSummary({
        total_items: 0,
        total_amount: 0,
      ...
      // Then remove each item on server
      const itemsToRemove = [...cartItems]; // Copy current items
  ...
  // **🎯 RESTORED Previous Orders Section - Complete with all functionality**
  const PreviousOrdersSection = () => {
    return (
      <div style={{
        backgroundColor: '#FFFFFF',
        ...
        {previousOrders.length > 0 ? (
                  ...
                  >
                    <div style={{ 
                      ...
                      <div style={{
                        ...
                        <FaFileInvoice size={22} color="#006064" />
                      </div>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{
                          ...
                          <span style={{
                            backgroundColor: order.status === 'Draft' ? '#FEF3CD' : 
                                            order.status === 'Open' ? '#E3F2FD' : 
                                            order.status === 'Submitted' ? '#E8F5E8' : '#F3E5F5',
                            color: order.status === 'Draft' ? '#B45309' : 
                                   order.status === 'Open' ? '#1565C0' : 
                                   order.status === 'Submitted' ? '#2E7D32' : '#7B1FA2',
                            fontSize: '12px',
                            fontWeight: '600',
                            padding: '4px 8px',
                            borderRadius: '12px'
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '14px', fontWeight: '600' }}>{order.formattedDate}</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '16px', fontWeight: '700', color: '#B12704' }}>{order.formattedAmount}</div>
                        </div>
                        <div>
                            ...
                            >
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{
                                  ...
                                  <span>Rate: {item.currency || 'K'} {parseFloat(item.rate || 0).toLocaleString('en-IN')}</span>
                                </div>
                              </div>
                              ...
                              <div style={{
                                ...
                                {item.currency || 'K'} {parseFloat(item.amount || 0).toLocaleString('en-IN')}
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div style={{
                        ...
                        <button
                          onClick={() => handleViewQuotation(order.orderNumber)}
                          style={{
                            padding: '10px 16px',
                            backgroundColor: '#007185',
                            color: 'white',
                        ...
                        <button
                          onClick={() => handleDownloadQuotation(order.orderNumber)}
                          style={{
                            padding: '10px 16px',
                            backgroundColor: '#FF9900',
                            color: 'white',
          ...
          <div style={{
            ...
            {ordersLoading ? (
              ...
              <>
                <FaFileInvoice size={48} color="#CCCCCC" style={{ marginBottom: '16px' }} />
                <h3 style={{ color: '#0F1111', marginBottom: '8px' }}>No Previous Orders</h3>
  ...
  // **🎯 Empty Cart State - FIXED to show "Cart is Empty"**
  if (!cartItems || cartItems.length === 0) {
    return (
      <>
        <NotificationBar />
        <div style={{
          ...
          {/* **🎯 ALWAYS Show Previous Orders even when cart is empty** */}
          <div style={{ maxWidth: '1400px', margin: '60px auto 0' }}>
            <PreviousOrdersSection />
          </div>
        </div>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Enhanced Header with Clear Cart Button */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
                  ...
                  >
                    ...
                    <div style={{ 
                      ...
                      <div style={{
                        ...
                        Qty: {item.qty} • Line Total: ₹{item.amount?.toLocaleString('en-IN')}
                      </div>
